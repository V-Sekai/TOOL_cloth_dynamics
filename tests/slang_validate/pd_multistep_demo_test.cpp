// Multi-step time integration over the same 2-vert 1-spring 1-pin
// system that pd_step_demo proved correct on a single step. This
// harness wraps the same six-kernel composition in an outer time
// loop with velocity tracking and damping, and validates that the
// system actually relaxes toward equilibrium under PD dynamics.
//
// System (identical to pd_step_demo):
//   v0  pinned at (0, 0, 0)     sqrtW_att    = 1
//   v1  spring to v0             sqrtW_spring = 1,  l0 = 1
//   x(0) = [(0,0,0), (2,0,0)],  v(0) = 0
//   mass[v] = 1,  no gravity
//
// Time integration (semi-implicit Euler + per-step velocity damping):
//   per step:
//     s_n    = x_n + dt · v_n               (predictor)
//     q      = PD_step(s_n)                  (six-kernel pipeline)
//     v_{n+1} = damp · (q − x_n) / dt        (damped velocity)
//     x_{n+1} = q
//
// Without damping a frictionless mass-spring would oscillate around
// equilibrium forever; with damp = 0.95 and dt = 0.1 the 2-vert
// system relaxes to its rest configuration (x = [(0,0,0), (1,0,0)])
// inside the 200-step budget.
//
// Final-state assertions:
//   |x_0| < 0.05                                (pin held v0 near 0)
//   ||x_1 − (1, 0, 0)|| < 0.05                  (spring near rest length)
//   |v_0| + |v_1| < 0.01                         (kinetic energy dissipated)
...
int main() {
    ...
    for (int step = 0; step < N_STEPS; ++step) {
        ...
        for (uint32_t i = 0; i < V; ++i) {
            ...
            s[3*i + 1] = positions[i][1] + dt * velocity[3*i + 1];
    ...
    // ---- Final-state assertions -------------------------------------------
    const float v0_dist  = std::sqrt(positions[0][0]*positions[0][0]
                                   + positions[0][1]*positions[0][1]
                                   + positions[0][2]*positions[0][2]);
    const float v1_dx    = positions[1][0] - 1.0f;
    const float v1_dy    = positions[1][1];
    ...
    int fails = 0;
    if (v0_dist > 0.05f)  { std::fprintf(stderr, "pin drift: |x0|=%g > 0.05\n", v0_dist); ++fails; }
    if (v1_dist > 0.05f)  { std::fprintf(stderr, "spring rest err: |x1-(1,0,0)|=%g > 0.05\n", v1_dist); ++fails; }
    if (kinetic > 0.01)   { std::fprintf(stderr, "residual kinetic: %g > 0.01\n", kinetic); ++fails; }

    const auto t1 = std::chrono::steady_clock::now();
    const double elapsed = std::chrono::duration<double>(t1 - t0).count();
    if (elapsed > kBudgetSeconds) {
        std::fprintf(stderr, "pd_multistep_demo: TIMEOUT — %.3fs > %.1fs\n",
                     elapsed, kBudgetSeconds);
        return 2;
    }
    if (fails == 0) {
        std::printf(
            "pd_multistep_demo: %d steps OK (spring %.4f → %.4f, "
            "x0_dist=%.3e, x1_dist_to_rest=%.3e, kin=%g, %.1fms)\n",
            N_STEPS, spring_len_initial, spring_len_final,
            v0_dist, v1_dist, kinetic, elapsed * 1000.0);
        return 0;
    }
    std::fprintf(stderr,
        "pd_multistep_demo: FAIL after %d steps (spring %.4f → %.4f, "
        "x0=[%g,%g,%g], x1=[%g,%g,%g])\n",
        N_STEPS, spring_len_initial, spring_len_final,
