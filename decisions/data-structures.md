...
### SpringBoneSegment Structure

The `SpringBoneSegment` represents a single element in a procedural animation chain, combining collision geometry with animation parameters.

**Definition:**
```cpp
struct SpringBoneSegment {
    TaperedCapsule* capsule;     // Associated collision primitive
    int parent_segment;          // Parent segment index (-1 for root)
    double stiffness;            // Animation stiffness parameter
    double damping;              // Motion damping coefficient
    Vec3d animation_offset;      // Additional procedural displacement
};
```

**Animation Characteristics:**
- **stiffness**: Controls responsiveness to movement (not physical spring constant)
- **damping**: Governs motion decay for natural-looking animation
- **animation_offset**: Additional displacement for secondary motion effects

### SpringBoneChain Structure

The `SpringBoneChain` implements hierarchical procedural animation systems for dynamic elements like hair, tails, and clothing.
...
public:
    ...
    // Construction
    static SpringBoneChain createProceduralChain(
        const Bone& source_bone,
...
- Segments form hierarchical chains with parent-child relationships
- Each segment combines collision detection with animation parameters
- Motion propagates through the chain with damping
- Enables realistic secondary motion for dynamic elements

**Key Features:**
- **Hierarchical Structure**: Tree-based organization of animation segments
- **Collision Integration**: Each segment includes collision geometry
- **Parameter Control**: Independent stiffness and damping per segment
- **Taper Support**: Radius reduction along chain length
