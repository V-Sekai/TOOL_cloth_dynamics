...
func _process(_delta):
	...
	var start_index = 0
	var end_index = 350
	var base_file_path = "output/rotating_sphere-randseed-123-20240924_195433-forwardThresh--9.0-LBFGS/iter24/"
	var gltf_path = "output/" + "rotating_sphere.gltf"
	
	var mesh_instance = MeshInstance3D.new()
	var array_mesh: ImporterMesh = ImporterMesh.new()
	mesh_instance.mesh = array_mesh
	
	var blend_shapes: Array[Array] = []
	
	for i in range(1, end_index):
		var obj_path = base_file_path + str(i) + ".obj"
		var mesh: ArrayMesh = load_obj(obj_path)
		if mesh == null:
			print("Cannot load: %d" % i)
			continue
		
		# Assume only one surface in OBJ format per mesh
		var surface_arrays = mesh.surface_get_arrays(0)
		var modified_arrays: Array = []
		modified_arrays.resize(Mesh.ARRAY_MAX)
		modified_arrays.fill(null)
		modified_arrays[Mesh.ARRAY_VERTEX] = surface_arrays[Mesh.ARRAY_VERTEX]
		modified_arrays[Mesh.ARRAY_NORMAL] = surface_arrays[Mesh.ARRAY_NORMAL]
		modified_arrays[Mesh.ARRAY_TANGENT] = surface_arrays[Mesh.ARRAY_TANGENT]
		
		blend_shapes.append(modified_arrays)
		array_mesh.add_blend_shape(str(i))
	
	var obj_path = base_file_path + str(0) + ".obj"
	var base_mesh: ArrayMesh = load_obj(obj_path)
	if base_mesh == null:
		print("Failed to load base mesh.")
		return
	
	# Assume only one surface in OBJ format per mesh
	var arrays = base_mesh.surface_get_arrays(0)
	array_mesh.add_surface(Mesh.PRIMITIVE_TRIANGLES, arrays, blend_shapes)
	mesh_instance.mesh = array_mesh.get_mesh()
	print("Blend count: %d" % array_mesh.get_blend_shape_count())
	print("Blend array count: %d" % blend_shapes.size())
	print("Surface count: %d" % array_mesh.get_surface_count())
	
	root.add_child(mesh_instance, true)
	mesh_instance.owner = root

	var gltf_exporter: GLTFDocument = GLTFDocument.new()
	var state: GLTFState = GLTFState.new()
	gltf_exporter.append_from_scene(root, state)
	gltf_exporter.write_to_filesystem(state, gltf_path)
